import React from 'react'
import { Button, Card, Col, Container, Row } from 'react-bootstrap';
import '../../components/ContainerVerifier.css';


function VerifierHomePage() {



  
  return (
    <div className='div'>
      <div className="divcontainer">
        <h1>Welcome</h1>
    <Row style={{flexWrap: 'wrap', width: '100%', height: 'auto', paddingTop: '30px', paddingBottom: '30px', padding: '15px'}}>
        <Col>
        
      <Card border="primary" style={{borderRadius: "10px", paddingTop: "25px", paddingBottom: "25px",padding: "15px"}}>
     <Card.Header >
  <Card.Img style={{maxWidth: "100%", paddingBottom: '15px'}} variant="top" src="https://mpng.subpng.com/20180421/obq/kisspng-injection-computer-icons-medicine-health-care-syri-shot-clipart-5adadcb3673e06.9413091315242927874229.jpg"/>
  </Card.Header>
  <Card.Body>
    <Card.Title>Required Vaccines</Card.Title>
    <Button href="/users/verifier/requiredVaccines"> Go To Vaccines</Button>
  </Card.Body>
</Card>
</Col>

<Col>
<Card border="primary" style={{borderRadius: "10px", paddingTop: "25px", paddingBottom: "25px",padding: "15px"}}>
<Card.Header>
  <Card.Img style={{maxWidth: "100%", paddingBottom: '15px'}} variant="top" src="data:image/png;base64," />
  </Card.Header>
  <Card.Body>
    <Card.Title>QR Scanner</Card.Title>
    <Button href="/users/verifier/QRScanner">Go to QR Scanner</Button>
  </Card.Body>
</Card>

</Col>
</Row>
</div>
    </div>
  
  )
}

export default VerifierHomePage
