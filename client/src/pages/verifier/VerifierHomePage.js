import React from 'react'
import { Button, Card, Col, Container, Row } from 'react-bootstrap';
import '../../components/ContainerVerifier.css';


function VerifierHomePage() {



  
  return (
    <Container>
    <div className='div'>
      <Container className='containerdiv '>
      <Row style={{width: '100%'}}>
        <Col>
   
      <Card border="primary" style={{borderRadius: "10px", paddingTop: "25px", paddingBottom: "25px",padding: "25px"}}>
     <Card.Header>
  <Card.Img style={{maxHeight: "250px", maxWidth: "100%"}} variant="middle" src="https://mpng.subpng.com/20180421/obq/kisspng-injection-computer-icons-medicine-health-care-syri-shot-clipart-5adadcb3673e06.9413091315242927874229.jpg"/>
  </Card.Header>
  <Card.Body>
    <Card.Title>Required Vaccines</Card.Title>
    <Button href="/users/verifier/requiredVaccines"> Go To Vaccines</Button>
  </Card.Body>
</Card>
</Col>

<Col>
<Card border="primary" style={{borderRadius: "10px", paddingTop: "25px", paddingBottom: "25px",padding: "25px"}}>
  <Card.Header>
  <Card.Img style={{maxHeight: "250px", maxWidth: "100%"}} variant="top" src="data:image/png;base64," />
  </Card.Header>
  <Card.Body>
    <Card.Title>QR Scanner</Card.Title>
    <Button href="/users/verifier/QRScanner">Go to QR Scanner</Button>
  </Card.Body>
</Card>
</Col>
</Row>
</Container>
    </div>
    </Container>
  )
}

export default VerifierHomePage
