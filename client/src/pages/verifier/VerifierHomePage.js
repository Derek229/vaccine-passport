import React from 'react'
import { Card, Col, Container, Row } from 'react-bootstrap';
import '../../components/Container.css';
import '../../components/VaccineCard.css'
function VerifierHomePage() {



  
  return (
    <div className='div'>
      <Container className='divcontainer'>
      <Row>
        <Col>
   
      <Card style={{ width: '18rem' }}>
  <Card.Img variant="middle" src="https://mpng.subpng.com/20180421/obq/kisspng-injection-computer-icons-medicine-health-care-syri-shot-clipart-5adadcb3673e06.9413091315242927874229.jpg"/>
  <Card.Body>
    <Card.Title>Required Vaccines</Card.Title>
    <Card.Link href="/users/verifier/requiredVaccines"> Go To Vaccines</Card.Link>
  </Card.Body>
</Card>
</Col>

<Col>
<Card className='card'>
  <Card.Img variant="top" src="data:image/png;base64," />
  <Card.Body>
    <Card.Title>QR Scanner</Card.Title>
  </Card.Body>
  <Card.Body>
    <Card.Link href="/users/verifier/QRScanner">Go to QR Scanner</Card.Link>
  </Card.Body>
</Card>
</Col>
</Row>
</Container>
    </div>
  )
}

export default VerifierHomePage
